...
class MY_Model:
  def __init__(self, sess):
    # # Convert doc_data to numpy array
    # doc_data_array = []
    # for sents_doc in doc_data:
    #     padding = [0] * FLAGS.sentembed_size
    #     sents_doc = sents_doc + [padding for _ in
    #                              range(FLAGS.max_doc_length-len(sents_doc))]
    #     doc_data_array.append(sents_doc)

    dtype = tf.float16 if FLAGS.use_fp16 else tf.float32
    # doc_data_array = np.array(doc_data_array)
    ...
    self.extractor_output, self.logits = model_docsum.policy_network(
        self.document_placeholder, self.label_placeholder)
